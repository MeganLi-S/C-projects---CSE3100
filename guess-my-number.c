...
void    child_main(int fdp[], int fdc[], int seed)
{
    ...
    close(fdc[0]); //child to write, so close read
    close(fdp[1]); //close writing end, read from parent
    int max = gmn_get_max();
    write(fdc[1], &max, sizeof(max)); //send max value to parent


    int guess;
    read(fdp[0], &guess, sizeof(guess)); /* read from pipe also wait */
    ...
    while(message[i] != '\n'){
        ...
        write(fdc[1], &letter, sizeof(letter));
        i++;
    }
...
int main(int argc, char *argv[])
{
    ...
    close(fdc[1]); //reading from child
    ...
    char letter;
    read(fdc[0], &letter, sizeof(letter));
    printf("%c", letter);
    while(letter != '\n'){
        read(fdc[0], &letter, sizeof(letter));
        printf("%c", letter);
