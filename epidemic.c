...
int location_match(node *head, THost host)
{
	node *traverse = head;
	while(traverse != NULL){
		if(traverse->host.x == host.x && traverse->host.y == host.y){
			return 1;
		}
		else{
...
int one_round(THost *hosts, int m, node *p_arr[], int n_arr, int k, int T)
{
    ...
    for(int i = 0; i < m; i++)
    {
        if(hosts[i].type == S) //Note the use of enumerator S
        {
            ...
            if(location_match(p_arr[index], hosts[i]))
            {
				...
				{
					hosts[i].type = I;
                	hosts[i].t = 0;//TODO: fill in what should happen here (not long)
					//node *insert = create_node(hosts[i]);
					//add_first(&p_arr[index], insert);
    ...
    for(int i = 0; i < m; i++){
		...
		if(p_arr[index] != NULL && p_arr[index]->next != NULL){
			remove_all(&p_arr[index]);
		}
		else{
			node *p;
			p = remove_first(&p_arr[index]);
			free(p);
		}
	}
